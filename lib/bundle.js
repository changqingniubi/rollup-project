 ...
 * @LastEditTime: 2021-07-07 18:02:07
...
class Bundle{
    ...
    generate(){
      ...
      let dependencies=[]
      this.statements.forEach(statement=>{
        dependencies.push(Object.keys(statement._dependsOn));
        dependencies.push(Object.keys(statement._dependsOwnOn));
      ...
      this.statements.forEach(statement=>{
        ...
        //取到所有的依赖的变量和定义的变量
        Object.keys(statement._dependsOn).concat(Object.keys(statement._dependsOwnOn)).concat(Object.keys(statement._defines))
        .forEach(name=>{
        ...
        if(/^Export/.test(statement.type)){
          if(statement.declaration.declarations){
            source.overwrite(statement.start,statement.declaration.declarations[0].start-1,'var');
          }else{
        ...
        //     source.remove(statement.start,statement.consequent.start);
        //   }else{
        //     var variableDeclaratorArray=[];
        //     walk(statement,{
        //       enter(node){
        //         switch(node.type){
        //           case 'VariableDeclaration':
        //             node.declarations.forEach((variableDeclarator)=>{
        //               if(node.kind === 'var' && dependencies.includes(variableDeclarator.id.name)){
        //                 variableDeclaratorArray.push(variableDeclarator.id.name);
        //               }
        //             })
        //           break;
        //         }
        //       }
        //     })
        //     source.overwrite(statement.start,statement.end,'var '+variableDeclaratorArray.join(',')+';');
        //   }
        // }
        if(statement.type==='VariableDeclaration'){
          if(statement.kind === 'let' || statement.kind === 'const'){
            //进行重写
            source.overwrite(statement.start,statement.declarations[0].start-1,'var');
          }
        }
        //用新名字替换掉老名字
